...
public class Ejercicio_02 {
    public static void main(String[]args){
        ...
        do{
            ...
            tec.reset();
            try{
                if (!input.equals("")) muestraInfoRuta(ruta, true);
        ...
        if(ruta.isFile()){
            ...
            for(int i = 0 ; i < lista.length ; i++)
                if (lista[i].isDirectory())
                    System.out.print("[*] - " + lista[i].getName() + ((info)?" - " + lista[i].length() + " - " + new Date(lista[i].lastModified()) + "\n":"\n"));
            
            System.out.println("\nArchivos:");
            for(int i = 0; i < lista.length ; i++)
                if (lista[i].isFile())
                    System.out.print("[A] - " + lista[i].getName() + ((info)?" - " + lista[i].length() + " - " + new Date(lista[i].lastModified()) + "\n":"\n"));
        }
    }
}
